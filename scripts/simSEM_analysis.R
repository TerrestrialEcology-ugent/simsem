...
setwd("~/Desktop/Blog/GitFolder/SimSEM/")
...
dd$best_scenario <- factor(dd$best_scenario,labels = c("none","underspecified","shuffled","exact","overspecified"))
...
dd_all$metric[dd_all$metric == "AIC"] <- "AICc"
...
dd_aic <- subset(dd,metric=="AIC" & !is.na(best_scenario))

gg_aic <- ggplot(dd_aic,aes(x=N,y=prop,color=best_scenario,linetype=pkg)) +
  geom_line() +
  facet_grid(X~Signal) +
  scale_x_log10() +
  labs(title="AICc",x="Sample size",y="Proportion of simulations")

ggsave("figures/aic_appendix.png",gg_aic, width = 10, height = 8)


dd_bic <- subset(dd,metric=="BIC" & !is.na(best_scenario))

gg_bic <- ggplot(dd_bic,aes(x=N,y=prop,color=best_scenario,linetype=pkg)) +
  geom_line() +
  facet_grid(X~Signal) +
  scale_x_log10() +
  labs(title="BIC",x="Sample size",y="Proportion of simulations")

ggsave("figures/bic_appendix.png",gg_bic,height=8,width=10)

dd_hbic <- subset(dd,metric=="hbic" & !is.na(best_scenario))

gg_hbic <- ggplot(dd_hbic,aes(x=N,y=prop,color=best_scenario,linetype=pkg)) +
  geom_line() +
  facet_grid(X~Signal) +
  scale_x_log10() +
  labs(title="HBIC",x="Sample size",y="Proportion of simulations")
